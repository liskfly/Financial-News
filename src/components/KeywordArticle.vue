<template>
  <div class="keyword" :style="{backgroundColor:type == 'authors' ? '#f6f6f6':'#fff'}">
    <div class="authors-top" v-if="keyword[0]" :style="{backgroundColor:type == 'authors' ? '#f6f6f6':''}">
    ...
    <div>
        ...
        <div class="personal">
          <div class="personal-top">
            <div class="personal-top-left">
              ...
              <div class="frequency">
                <div>{{keyword[0].authors[index].articles_count + '篇文章'}}</div>
                <div class="line">|</div>
                <div>{{'关注'}}</div>
              </div>
              <wd-button size="small" class="follow">关注</wd-button>
            </div>
            <img :src="keyword[0].authors[index].avatar" class="head-portrait">
          </div>
      ...
      <div class="news">
        <ul>
          <li v-for="{title,cover_url,id} in keyword" :key="id">
            <div class="show-pic">
...
export default {
  ...
  methods: {
    ...
    getKeywordArticle () {
      this.$axios
        .get(`http://api2021.cbnweek.com/v4/${this.type}/${this.id}/articles?page=1&per=20`)
        .then(({ data }) => {
          this.keyword = data
          this.authorsIndex()
          console.log(this.keyword, 1, this.index);
        })
    },
    authorsIndex () {
