<template>
  <div class="audio-play" v-if="audioCon">
    <div
      class="bg"
      :style="{ backgroundImage: 'url(' + audioCon[0].cover_url + ')' }"
    >
      <div class="audio-box">
        <div class="head">
        ...
        <div class="audio-anime">
          <div class="run" ref="runAnim">
            <img :src="audioCon[0].cover_url" alt="tp" />
          </div>
        </div>
        <div class="control">
          <van-slider
          ...
          <div class="audio-control">
            ...
            >
              <div class="progm">
                ...
                <span>共{{ audioArr.length }}集</span>
              </div>
              <div class="progm-box" v-if="audioArr">
                <div
                  class="audio-item"
                  v-for="(a, i) in audioArr"
                  :key="a.title"
...
export default {
  ...
  methods: {
    ...
    preAudio(id) {
      ...
      let a = this.audioArr.findIndex((t) => t.id == id);
      if (a == 0) {
        alert("这是第一个Audio");
      } else {
        let preId = this.audioArr[a - 1].id;
        this.$emit("sent-play", { audioid: preId, isPlay: this.isPlay });
      }
    },
    nextAudio(id) {
      let a = this.audioArr.findIndex((t) => t.id == id);
      if (a == this.audioArr.length - 1) {
        alert("这是最后一个Audio");
      } else {
        let preId = this.audioArr[a + 1].id;
        this.$emit("sent-play", { audioid: preId, isPlay: this.isPlay });
