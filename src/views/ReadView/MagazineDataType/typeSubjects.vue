<template>
  <div class="bookContent">
    ...
    <ul class="comment">
      <li v-for="{id,introduction,avatar,name} in bookData.authors" :key="id" class="comment-list"
    ...
    <ul class="newest">
      <li v-for="{id,title,summary,display_time,read_time} in bookData.articles" :key="id" class="newest-list">
        <span class="title">{{title}}</span>
        <span class="summary">{{summary}}</span>
        <span class="time">{{getTime(display_time) + '·阅读时长'+ read_time + '分钟'}}</span>
      </li>
    </ul>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    goKeyWordArticle (id) {
      this.$router.push(
        `/keyword-article?type=authors&id=${id}`
      )
    }
  },
}
